...
$cek_penunjang = mysqli_query($koneksi, "SELECT $bulan,breakdown_id,$database_skp.rwyt_target_kerja_tahunan.* FROM $database_skp.rwyt_breakdown_target_kerja 
				LEFT JOIN $database_skp.rwyt_target_kerja_tahunan ON $database_skp.rwyt_target_kerja_tahunan.target_id=$database_skp.rwyt_breakdown_target_kerja.target_id 
				WHERE $database_skp.rwyt_breakdown_target_kerja.pegawai_id='$j[pegawai_id]' 
				AND $database_skp.rwyt_breakdown_target_kerja.informasi_id='$l[informasi_id]' 
				AND unsur_id=2 ORDER BY $database_skp.rwyt_breakdown_target_kerja.target_id");
$ketemu_penunjang = mysqli_num_rows($cek_penunjang);

if ($ketemu_penunjang > 0) {
    $tampil = mysqli_query($koneksi, "SELECT $bulan,breakdown_id,$database_skp.rwyt_target_kerja_tahunan.* FROM $database_skp.rwyt_breakdown_target_kerja 
					LEFT JOIN $database_skp.rwyt_target_kerja_tahunan ON $database_skp.rwyt_target_kerja_tahunan.target_id=$database_skp.rwyt_breakdown_target_kerja.target_id 
					WHERE $database_skp.rwyt_breakdown_target_kerja.pegawai_id='$j[pegawai_id]' 
					AND $database_skp.rwyt_breakdown_target_kerja.informasi_id='$l[informasi_id]' 
        ...
        if ($r[$bulan] <> '0') {
            ...
            if ($l['jabatan_id'] == '1') {
                if ($r['kegiatan_id'] == '1' or $r['kegiatan_id'] == '2') {
                    $tugas_sql = mysqli_query($koneksi, "SELECT * from $database_skp.ref_dau_dak WHERE id='$r[program_keg_subkeg]'");
                    $tugas = mysqli_fetch_array($tugas_sql);
                    $record['kegiatan_tugas_jabatan'] = "$tugas[name] ($r[program_keg_subkeg])";
                } else {
                    $tugas_sql = mysqli_query($koneksi, "SELECT * FROM $database_skp.ref_js_uraian_tugas
									LEFT JOIN $database_skp.ref_satuan ON $database_skp.ref_satuan.satuan_id=$database_skp.ref_js_uraian_tugas.satuan_id
									WHERE uraian_id='$r[tugas_jab]'");
            ...
            //-------------target----------------------------------------------------------
            $ak_bulan_limpahan = $tugas['angka_kredit'] * $r[$bulan];
            $record['ak'] = $ak_bulan_limpahan;
            ...
            if ($r['kegiatan_id'] == '1' or $r['kegiatan_id'] == '2') {
                ...
                $target['kuantitas'] = "$r[$bulan] $tugas[satuan]";
                $wp_bulan_limpahan = $r['wp'] * $r[$bulan];
            }
            ...
            $tampil1 = mysqli_query($koneksi, "SELECT $database_skp.rwyt_realisasi_kuantitas.* FROM $database_skp.rwyt_realisasi_kuantitas 
							...
							AND $database_skp.rwyt_realisasi_kuantitas.breakdown_id='$r[breakdown_id]' 
							AND $database_skp.rwyt_realisasi_kuantitas.informasi_id='$r[informasi_id]' 
							AND unsur_id=2 ORDER BY $database_skp.rwyt_realisasi_kuantitas.target_id");
            $s = mysqli_fetch_array($tampil1);

            $realisasi = [];
            if ($r['kegiatan_id'] == '1' or $r['kegiatan_id'] == '2') {
                $realisasi['kuantitas'] = "$r[kuantitas] $tugas[satuan]";
            } else {
                if ($s[$bulan] == '') {
                    $realisasi['kuantitas'] = "";
                } else {
            ...
            $wp_bulan_limpahan1 = $wp_bulan_limpahan1 + $wp_bulan_limpahan;
            $ak_bulan_limpahan1 = $ak_bulan_limpahan1 + $ak_bulan_limpahan;
...
$cek_lain = mysqli_query($koneksi, "SELECT $bulan,breakdown_id,$database_skp.rwyt_target_kerja_tahunan.* FROM $database_skp.rwyt_breakdown_target_kerja 
                LEFT JOIN $database_skp.rwyt_target_kerja_tahunan ON $database_skp.rwyt_target_kerja_tahunan.target_id=$database_skp.rwyt_breakdown_target_kerja.target_id 
                WHERE $database_skp.rwyt_breakdown_target_kerja.pegawai_id='$j[pegawai_id]' 
                AND $database_skp.rwyt_breakdown_target_kerja.informasi_id='$l[informasi_id]' 
                AND unsur_id=3 ORDER BY $database_skp.rwyt_breakdown_target_kerja.target_id");
$ketemu_lain = mysqli_num_rows($cek_lain);
...
if ($ketemu_lain > 0) {
    ...
    while ($r = mysqli_fetch_array($tampil)) {
        if ($r[$bulan] <> '0') {
            ...
            if ($l['jabatan_id'] == '2') {
                $tugas_sql = mysqli_query($koneksi, "SELECT * FROM $database_skp.ref_jft_uraian_penunjang 
                                LEFT JOIN $database_skp.ref_satuan ON $database_skp.ref_satuan.satuan_id=$database_skp.ref_jft_uraian_penunjang.satuan_id 
                                WHERE uraian_id='$r[tugas_jab]'");
                $tugas = mysqli_fetch_array($tugas_sql);
                $record['kegiatan_tugas_jabatan'] = "$tugas[uraian] (AK:$tugas[angka_kredit])";
            }
            ...
            //-------------target----------------------------------------------------------
            $ak_bulan_penunjang = $tugas['angka_kredit'] * $r[$bulan];
            $record['ak'] = $ak_bulan_penunjang;
            ...
            if ($r['kegiatan_id'] == '1' or $r['kegiatan_id'] == '2') {
                ...
                $target['kuantitas'] = "$r[$bulan] $tugas[satuan]";
                $wp_bulan_penunjang = $r['wp'] * $r[$bulan];
            }
            ...
            $tampil1 = mysqli_query($koneksi, "SELECT $database_skp.rwyt_realisasi_kuantitas.* FROM $database_skp.rwyt_realisasi_kuantitas 
                            ...
                            AND $database_skp.rwyt_realisasi_kuantitas.breakdown_id='$r[breakdown_id]' 
                            AND $database_skp.rwyt_realisasi_kuantitas.informasi_id='$r[informasi_id]' 
                            AND unsur_id=3 ORDER BY $database_skp.rwyt_realisasi_kuantitas.target_id");
            $s = mysqli_fetch_array($tampil1);
            ...
            $wp_bulan_penunjang1 = $wp_bulan_penunjang1 + $wp_bulan_penunjang;
            $ak_bulan_penunjang1 = $ak_bulan_penunjang1 + $ak_bulan_penunjang;
